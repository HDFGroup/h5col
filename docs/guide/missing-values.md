...
is a single canonical test that every consumer applies:
...
Both styles, a sentinel that compares equal or NaN, which never compares
equal to itself, reduce to this one rule. The package applies it for you:
...
receives the convention's recommended sentinel for its datatype:
...
Two of these deserve a note. The floating-point sentinel is not `NaN` but
the value long used as the default fill in the netCDF world, kept here so
files behave consistently across that ecosystem. `NaN` remains available as an
...
A sentinel only works if no genuine value can equal it. Declaring
...
is raised. In the `samples` spec above, `-1` is provably not a value, because
values start at 0.
...
consequence worth internalizing is that `field("x") == 5` and its negation
